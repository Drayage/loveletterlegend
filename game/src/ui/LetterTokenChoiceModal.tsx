...
interface LetterTokenChoiceModalProps {
  ...
  /** 배치 사유 (점술사 공동 승리 등) -- 없으면 라운드 승리 보상. */
  reason?: string;
...
export function LetterTokenChoiceModal({ amount, atCap, tokens, availableSlots, reason, onChoose }: LetterTokenChoiceModalProps) {
  const [moveFrom, setMoveFrom] = useState<CharacterSlotId | null>(null);
  const allSlots = Object.keys(SLOT_INFO) as CharacterSlotId[];

  if (!atCap) {
    return (
      <Modal title={reason ?? "라운드 승리!"} onClose={() => {}} dismissible={false}>
