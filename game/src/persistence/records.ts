...
export interface GameRecords {
  ...
  /** CharacterSlotId -> 그 상대와 맺어져 세션을 마친 횟수. */
  endingsAchieved: Partial<Record<CharacterSlotId, number>>;
  lastPlayedAt: string | null;
}

function defaultRecords(): GameRecords {
  return { version: 1, sessionsPlayed: 0, endingsAchieved: {}, lastPlayedAt: null };
}
...
/** 세션이 끝날 때 한 번 호출 -- 사람 플레이어가 맺어진 상대(없으면 null)를
 * 기록에 누적한다. */
export function recordSessionEnding(endingSlot: CharacterSlotId | null): GameRecords {
