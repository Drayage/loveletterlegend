...
### Phase 1 — 코어 엔진 + 기본 16장 (MVP)
- 표준 러브레터 규칙 엔진: 덱 셔플/드로우, 손패 2장 중 1장 플레이, 각 기본 카드(경비병~공주) 효과 구현, 탈락/라운드 종료/승자 판정.
- 2인용(나 vs AI) 로컬 플레이. 온라인 없음.
- **AI 로직**: 상대가 낸 카드/지목 이력으로 각 플레이어가 들고 있을 카드 확률 테이블 유지 → 각 선택지의 기대 생존율/승률로 최선의 수 선택. (전형적인 러브레터 AI 휴리스틱, 새로 연구할 필요 없음)
- UI: 손패, 덱, 버림 더미, 현재 라운드 상태만. 스토리 요소 없음.
- **완료 기준**: 기본 16장으로 1라운드를 처음부터 끝까지 AI와 완전히 플레이 가능.
